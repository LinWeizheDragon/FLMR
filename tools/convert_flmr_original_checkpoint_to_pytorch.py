...
HFCheckpointState = collections.namedtuple("CheckpointState", ["state_dict"])

def load_states_from_hf_checkpoint(model_file: str) -> HFCheckpointState:
    print(f"Reading saved model from {model_file}")
    state_dict = torch.load(model_file, map_location=lambda s, l: default_restore_location(s, "cpu"))
    state_dict = {"state_dict": state_dict}
    return HFCheckpointState(**state_dict)
...
class FLMRState:
    ...
    def from_type(comp_type: str, *args, **kwargs) -> "FLMRState":
        ...
        elif comp_type.startswith("PreFLMR"):
            return PreFLMRModelForRetrievalState(*args, **kwargs)
        elif comp_type.startswith("ColBERT"):
            return ColBERTState(*args, **kwargs)
        else:
            raise ValueError("Component type must be either 'FLMR', 'PreFLMR' or 'ColBERT'.")
...
class ColBERTState(FLMRState):
    def load_flmr_model(self, **kwargs):
        query_tokenizer = FLMRQueryEncoderTokenizer.from_pretrained(self.src_file)
        context_tokenizer = FLMRContextEncoderTokenizer.from_pretrained(self.src_file)

        text_config = FLMRTextConfig.from_pretrained(self.src_file)
        vision_config = FLMRVisionConfig.from_pretrained(self.vision_model_version)
        ...
        src_file = os.path.join(self.src_file, "pytorch_model.bin")
        print(f"Loading PreFLMR biencoder from {src_file}")
        saved_state = load_states_from_hf_checkpoint(src_file)
        ...
        for key, value in saved_state.state_dict.items():
            if key.startswith("bert."):
                new_key = key.replace("bert.", "context_text_encoder.bert_model.")
            elif key.startswith("linear."):
                new_key = key.replace("linear.", "context_text_encoder_linear.")
            else:
                new_key = key
